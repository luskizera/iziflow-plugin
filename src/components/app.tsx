...
export function App() {
  ...
  useEffect(() => {
    ...
    const cleanupParseError = listenTS("parse-error", (payload) => {
      ...
      setError(
        `Erro de sintaxe no Markdown${payload.lineNumber ? ` (linha ${payload.lineNumber})` : ""}: ${payload.message}`
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      console.log(">>> Enviando para plugin:", {
        markdown,
        mode: nodeMode,
      ...
      setError(
        `Erro interno ao enviar dados para o plugin: ${dispatchError.message}`
      );
  ...
  const handleInputChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!newValue.startsWith("#")) {
      ...
      newValue = "#" + newValue.substring(1).replace(/[^0-9a-fA-F]/g, "");
    }
  ...
  return (
    <TooltipProvider delayDuration={100}>
      <div
        className={cn(
          "flex flex-col h-screen p-4 gap-4 bg-background text-foreground",
          uiTheme
        )}
      >
        {/* --- Header --- */}
              ...
              >
                ...
                <Tooltip>
                  ...
                  <TooltipContent side="top" align="center">
                    <p className="text-xs">
                      Define a cor de destaque dos nós.
                    </p>
                  </TooltipContent>
                </Tooltip>
              </Label>
            ...
            <div className="flex flex-1 flex-col items-start gap-1.5">
              ...
              >
                <TabsList className="flex-1 w-full p-1 bg-secondary border border-border rounded-md">
                  <TabsTrigger
                    value="dark"
                    className="flex flex-1 items-center gap-1.5 px-2 py-1 text-xs h-7 data-[state=active]:bg-background data-[state=active]:shadow-sm data-[state=inactive]:opacity-70"
