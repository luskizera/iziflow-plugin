...
import { dispatchTS, listenTS } from "@/utils/utils"; // Importa ambas as funções
import type { EventTS } from "@shared/types/messaging.types"; // Tipos para mensagens
import { SunIcon, MoonIcon, SettingsIcon, InfoIcon, Trash2Icon } from "lucide-react"; // Adicionado Trash2Icon
import { Label } from "@/components/ui/label";
import { Tabs, TabsList, TabsTrigger, TabsContent } from "@/components/ui/tabs"; // Importa TabsContent
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
import { ScrollArea } from "@/components/ui/scroll-area"; // Para lista de histórico

type NodeGenerationMode = "light" | "dark";

// Função isValidHex movida para fora do componente
...
export function App() {
  ...
  // --- LISTENER DE MENSAGENS E BUSCA DE HISTÓRICO INICIAL ---
  useEffect(() => {
    markdownTextareaRef.current?.focus();
    console.log("UI: Montado. Pedindo histórico inicial...");
    dispatchTS('get-history'); // <<< CORRIGIDO: Sem payload {}

    // Callbacks definidas separadamente
    const handleGenerationSuccess = (payload: EventTS['generation-success']) => {
      console.log("UI: Recebido 'generation-success'. Payload:", payload);
      setError(null);
      setIsLoading(false);
      // Pede histórico atualizado APÓS sucesso
      dispatchTS('get-history'); // <<< CORRIGIDO: Sem payload {}
    };
    const handleGenerationError = (payload: EventTS['generation-error']) => {
      console.error("UI: Recebido 'generation-error'. Payload:", payload);
      setError(`Erro na geração: ${payload.message}`);
      setIsLoading(false);
    };
    const handleParseError = (payload: EventTS['parse-error']) => {
      console.error("UI: Recebido 'parse-error'. Payload:", payload);
    ...
    const handleDebug = (payload: EventTS['debug']) => {
      // O log já acontece no plugin, apenas mostra no console da UI se quiser
      // console.debug(`UI [Plugin Debug]: ${payload.message}`, payload.data);
    };
    const handleHistoryData = (payload: EventTS['history-data']) => {
       console.log("UI: Recebido 'history-data'. Payload:", payload);
       // Garante que payload.history é um array antes de setar
       if (Array.isArray(payload.history)) {
           setHistory(payload.history);
           console.log("UI: Estado 'history' atualizado com", payload.history.length, "itens.");
       } else {
           console.error("UI: Formato de histórico inválido recebido:", payload);
           setHistory([]); // Reseta para vazio se inválido
       }
    };

    // Configura listeners individuais (passando apenas nome e callback)
    const cleanupSuccess = listenTS('generation-success', handleGenerationSuccess);
  ...
  const handleSubmit = async () => {
    ...
    if (!markdown.trim()) { setError("O campo Markdown não pode estar vazio."); setIsLoading(false); return; }
    ...
    try {
       ...
       dispatchTS("generate-flow", { markdown, mode: nodeMode, accentColor: finalAccentColor }); // Payload necessário aqui
    } catch (dispatchError: any) { setError(`Erro interno ao enviar dados: ${dispatchError.message}`); setIsLoading(false); }
  };
  ...
  const handleInputChange = (event: ChangeEvent<HTMLInputElement>) => {
      ...
      if (isValidHex(upperNewValue)) { setAccentColor(upperNewValue); }
  };
   ...
   const handleHistoryClick = (markdownItem: string) => {
        setSelectedHistoryItem(markdownItem);
        setIsDialogOpen(true);
    };

    const handleConfirmLoad = () => {
        if (selectedHistoryItem !== null) {
            setMarkdown(selectedHistoryItem);
            setError(null);
    ...
    // Handler para limpar histórico
     const handleClearHistory = () => {
         // Adiciona confirmação antes de enviar o pedido
         if (window.confirm("Tem certeza que deseja limpar todo o histórico? Esta ação não pode ser desfeita.")) {
             console.log("UI: Enviando pedido para limpar histórico...");
             dispatchTS('clear-history-request'); // <<< CORRIGIDO: Sem payload {}
             // A UI será atualizada automaticamente pelo listener 'history-data'
         } else {
             console.log("UI: Limpeza de histórico cancelada pelo usuário.");
         }
     }
  ...
  return (
    <TooltipProvider delayDuration={100}>
      ...
      <div className={cn("flex flex-col h-screen bg-background text-foreground", uiTheme)}>
        {/* Header */}
        <header className="flex items-center justify-between w-full flex-shrink-0 p-4 border-b border-border">
           {/* Logo SVG */}
           ...
           <div className="flex items-center gap-1.5">
               ...
               <Button variant="default" size="sm" className="p-0 w-8 h-8" title="Configurações" onClick={() => setActiveTab('settings')}>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col flex-grow min-h-0">
          <TabsList className="flex-shrink-0 mx-4 mt-4 mb-2 h-9 w-auto p-1 justify-start rounded-lg bg-muted text-muted-foreground">
            <TabsTrigger value="generator" className="flex-1 px-3 py-1 h-7 text-sm data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm">Gerador</TabsTrigger>
            <TabsTrigger value="settings" className="flex-1 px-3 py-1 h-7 text-sm data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm">Histórico</TabsTrigger>
          </TabsList>

          {/* --- Conteúdo da Aba Gerador --- */}
          <TabsContent value="generator" className="flex flex-col flex-grow p-4 gap-4 overflow-y-auto data-[state=inactive]:hidden">
            {/* Textarea */}
            <Textarea
              ref={markdownTextareaRef} value={markdown} onChange={(e) => setMarkdown(e.target.value)}
              placeholder="Cole seu IziFlow Markdown aqui ou comece a digitar..."
              className="flex-grow w-full resize-none font-mono text-sm min-h-[20vh] bg-muted/30 dark:bg-muted/10 border-border"
            />
            {/* Seção de Customização */}
            <div className="flex flex-col gap-3 w-full flex-shrink-0">
              <h2 className="text-xl font-medium">Customize os nós</h2>
              <div className="flex flex-row items-end gap-3">
                {/* Accent Color Input */}
                <div className="flex flex-1 flex-col items-start gap-1.5">
                   <Label htmlFor="accent-color-input" className="text-sm font-medium flex items-center gap-1">
                      Cor de Destaque
                      <Tooltip><TooltipTrigger asChild><InfoIcon className="w-3 h-3 text-muted-foreground cursor-help" /></TooltipTrigger><TooltipContent side="top" align="center"><p className="text-xs">Define a cor de destaque (HEX).</p></TooltipContent></Tooltip>
                  </Label>
                  <div className="relative flex items-end w-full h-9">
                      <Input id="accent-color-input" type="text" value={inputValue} onChange={handleInputChange} onBlur={handleInputBlur} className={cn("h-9 w-full pl-8 pr-2 text-xs font-sans", !isValidHex(inputValue) && "border-destructive focus-visible:border-destructive focus-visible:ring-destructive/30")} maxLength={7} aria-label="Accent color hex value" />
                      <div aria-hidden="true" className="absolute left-1.5 top-1/2 -translate-y-1/2 w-5 h-5 rounded border border-input pointer-events-none" style={{ backgroundColor: isValidHex(accentColor) ? accentColor : '#CCCCCC' }} />
                  </div>
                </div>
                {/* Node Mode Tabs */}
                <div className="flex flex-1 flex-col items-start gap-1.5">
                    <Label htmlFor="node-theme-tabs" className="text-sm font-medium flex items-center gap-1">
                      Tema dos Nós
                      <Tooltip><TooltipTrigger asChild><InfoIcon className="w-3 h-3 text-muted-foreground cursor-help" /></TooltipTrigger><TooltipContent side="top" align="center"><p className="text-xs">Tema visual dos nós gerados.</p></TooltipContent></Tooltip>
                    </Label>
                    <Tabs id="node-theme-tabs" value={nodeMode} onValueChange={handleNodeModeChange} className="w-full">
                      <TabsList className="flex-1 w-full p-1 bg-secondary border border-border rounded-md">
                        <TabsTrigger value="dark" className="flex flex-1 items-center gap-1.5 px-2 py-1 text-xs h-7 data-[state=active]:bg-background data-[state=active]:shadow-sm data-[state=inactive]:opacity-70" aria-label="Generate in Dark mode"><MoonIcon className="w-3.5 h-3.5"/> Nós Escuros</TabsTrigger>
                        <TabsTrigger value="light" className="flex flex-1 items-center gap-1.5 px-2 py-1 text-xs h-7 data-[state=active]:bg-background data-[state=active]:shadow-sm data-[state=inactive]:opacity-70" aria-label="Generate in Light mode"><SunIcon className="w-3.5 h-3.5"/> Nós Claros</TabsTrigger>
                      </TabsList>
                    </Tabs>
                </div>
              </div>
            ...
            <div className="w-full mt-auto flex-shrink-0 space-y-2 pt-2">
              {error && ( <Alert variant="destructive" className="text-xs"><AlertDescription>{error}</AlertDescription></Alert> )}
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={handleCleanText} title="Limpar área de texto">Limpar</Button>
          ...
          <TabsContent value="settings" className="flex flex-col flex-grow p-4 gap-4 overflow-hidden data-[state=inactive]:hidden">
            <h2 className="text-xl font-medium flex-shrink-0">Histórico de Markdown</h2>
            <div className="flex-grow min-h-0 border rounded-md bg-muted/30 dark:bg-muted/10">
              <ScrollArea className="h-full p-2">
                {history.length > 0 ? (
                    ...
                    {history.map((item, index) => (
                      <li key={index} className="text-xs font-mono">
                        <Button
                          ...
                          className="w-full justify-start h-auto py-1 px-2 text-left whitespace-pre-wrap hover:bg-background/50 dark:hover:bg-background/10"
                          onClick={() => handleHistoryClick(item)}
                          title="Clique para carregar este histórico"
                        >
                          {/* Mostra um snippet do histórico */}
                  ...
                  <p className="text-sm text-muted-foreground text-center py-4">Nenhum histórico salvo.</p>
                )}
              </ScrollArea>
            </div>
        ...
        {/* --- Alert Dialog (Fora das Abas) --- */}
        <AlertDialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Carregar Histórico?</AlertDialogTitle>
              <AlertDialogDescription>
                Isso substituirá o conteúdo atual da área de texto. Deseja continuar?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="flex justify-end gap-2 pt-4">
              <AlertDialogCancel onClick={handleCancelLoad}>Cancelar</AlertDialogCancel>
              <AlertDialogAction onClick={handleConfirmLoad}>Carregar</AlertDialogAction>
            </div>
          </AlertDialogContent>
        </AlertDialog>
      </div>
